...
export function FestivalBannerModal({ banners }: { banners: Banner[] }) {
  ...
  function close() {
    if (banner) sessionStorage.setItem(`charnish-banner-seen-${banner.id}`, 'true');
    setOpen(false);
  }

  return (
    <div className="fixed inset-0 z-[70] grid place-items-center bg-choc-900/78 p-4 backdrop-blur-md">
      <section className="relative w-full max-w-3xl overflow-hidden rounded-2xl bg-[#f5f3f7] shadow-[0_30px_120px_rgba(0,0,0,0.45)]">
        <button
          onClick={close}
        ...
        <div className="relative aspect-[16/8.6] min-h-[250px] bg-cream-dark">
        ...
        <div className="px-6 pb-8 pt-5 text-center md:px-12">
          <h2 className="font-heading text-4xl font-black leading-tight text-choc-800 md:text-5xl">{banner.title}</h2>
